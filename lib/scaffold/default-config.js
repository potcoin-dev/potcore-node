...
/**
 * Will return the path and default potcore-node configuration. It will search for the
 * configuration file in the "~/.potcoin" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.potcoin');
  var defaultConfigFile = path.resolve(defaultPath, './potcore-node.json');
      ...
      servicesConfig: {
        bitcoind: {
          spawn: {
            ...
            exec: path.resolve(__dirname, '../../bin/potcoind')
          }
        }
      }
    };
