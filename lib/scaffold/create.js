...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'potcore-lib': '^' + bitcore.version,
    'potcore-node': version
  }
};
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/potcore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the potcore node configuration directory
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the potcore-node directory and configuration
