...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and potcore-node.json. \n\n' +
 ...
 * @param {Object} options.config - The parsed potcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './potcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in potcore-node services directory
